...
public class SeedController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedAccPendingItems()
    {
        ...
        {
                new AccPendingItem { Name = "Dell Ultrasharp Monitor", Division = "Design", Date = DateTime.UtcNow.AddDays(-10), Status = "Pending", Category = AccPendingCategory.Pending, Time = new TimeSpan(14,15,0), AssetType = "Monitor", CurrentUser = "Jane Smith", SpecialNote = "Color accuracy is gone.", ValueAtPurchasing = 800m, CurrentValue = 150m },
                new AccPendingItem { Name = "Logitech MX Master 3", Division = "Design", Date = DateTime.UtcNow.AddDays(-9), Status = "Pending", Category = AccPendingCategory.Pending, Time = new TimeSpan(10,0,0), AssetType = "Peripheral", CurrentUser = "Jane Smith", SpecialNote = "Scroll wheel broken.", ValueAtPurchasing = 100m, CurrentValue = 0m },
                new AccPendingItem { Name = "Standing Desk", Division = "HR", Date = DateTime.UtcNow.AddDays(-8), Status = "Pending", Category = AccPendingCategory.Pending, Time = new TimeSpan(9,30,0), AssetType = "Furniture", CurrentUser = "Alice Brown", SpecialNote = "Motor burnt out.", ValueAtPurchasing = 500m, CurrentValue = 50m },
                new AccPendingItem { Name = "Epson Projector", Division = "Sales", Date = DateTime.UtcNow.AddDays(-7), Status = "Pending", Category = AccPendingCategory.Pending, Time = new TimeSpan(11,0,0), AssetType = "Electronics", CurrentUser = "Bob Wilson", SpecialNote = "Lamp needs replacement.", ValueAtPurchasing = 1500m, CurrentValue = 200m },
                new AccPendingItem { Name = "iPad Pro 12.9", Division = "Marketing", Date = DateTime.UtcNow.AddDays(-6), Status = "Pending", Category = AccPendingCategory.Pending, Time = new TimeSpan(15,20,0), AssetType = "Tablet", CurrentUser = "Sarah Connor", SpecialNote = "Cracked screen.", ValueAtPurchasing = 1100m, CurrentValue = 100m },
            };
        _context.AccPendingItems.AddRange(items); await _context.SaveChangesAsync(default); return Ok($"Seeded {items.Count} AccPendingItems (Pending:5).");
    }
